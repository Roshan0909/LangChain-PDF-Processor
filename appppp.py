...
def extract_text(file, file_type=None):
    """
    Extract text content from various file types.
    """
    if not file_type:
        file_type, _ = mimetypes.guess_type(file.name)
    file_type = file_type.split("/")[-1] if file_type else "unknown"

    try:
        if file_type == "pdf":
            return extract_text_from_pdf(file)
        elif file_type == "plain" or file_type == "txt":
            return extract_text_from_txt(file)
        elif file_type == "csv":
            return extract_text_from_csv(file)
        elif file_type == "vnd.openxmlformats-officedocument.spreadsheetml.sheet" or file_type == "xlsx":
            return extract_text_from_excel(file)
        else:
            raise ValueError("Unsupported file type or format.")
    except Exception as e:
        raise RuntimeError(f"Failed to process {file_type} file: {str(e)}")
